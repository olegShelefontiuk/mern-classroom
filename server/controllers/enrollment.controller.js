...
const findEnrollment = async(req,res,next) =>{
    try{
        let enrollments = await Enrollment.find({
            course: req.course._id, student : req.auth._id})
...
const listEnrolled = async (req,res) => {
    try{
        let enrollments = await
            Enrollment.find({student: req.auth._id})
                .sort({'completed': 1 })
...
module.exports = {remove, enrollmentStats, findEnrollment,create, enrollmentByID, isStudent, read, complete, listEnrolled}